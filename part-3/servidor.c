...
void init_database() {
    ...
    // • Lê o ficheiro FILE_CIDADAOS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->cidadaos e db->num_cidadaos. Se não o conseguir, dá erro e termina com exit status 1;
    int size_file = read_binary(FILE_CIDADAOS,db->cidadaos,MAX_CIDADAOS*sizeof(Cidadao));
    db->num_cidadaos = size_file / sizeof(Cidadao);


    // • Lê o ficheiro FILE_ENFERMEIROS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->enfermeiros e db->num_enfermeiros. Se não o conseguir, dá erro e termina com exit status 1;
    size_file = read_binary(FILE_ENFERMEIROS,db->enfermeiros,MAX_ENFERMEIROS*sizeof(Enfermeiro));
    db->num_enfermeiros = size_file / sizeof(Enfermeiro);
...
void processa_pedido() {
    ...
    int index_cidadao=-1;
    for(int i=0; i!=db->num_cidadaos && index_cidadao==-1; i++)
        if(mensagem.dados.num_utente==db->cidadaos[i].num_utente && strcmp(mensagem.dados.nome,db->cidadaos[i].nome) == 0)
            index_cidadao=i;
    ...
    //       • Se o Cidadão na BD Cidadãos tiver estado_vacinacao = 2 => status = VACINADO;
        if(db->cidadaos[index_cidadao].estado_vacinacao==2)
    ...
    //       • Se o Cidadão na BD Cidadãos tiver PID_cidadao > 0 => status = EMCURSO; caso contrário, afeta o PID_cidadao da BD Cidadãos com o valor do PID_cidadao da mensagem;
        else if(db->cidadaos[index_cidadao].PID_cidadao>0){
    ...
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
        sucesso("S5.1) Cidadão %d, %s encontrado, estado_vacinacao=%d, status=%d", mensagem.dados.num_utente, mensagem.dados.nome, db->cidadaos[index_cidadao].estado_vacinacao, resposta.dados.status);
    }
    ...
    if(resposta.dados.status != DESCONHECIDO && resposta.dados.status != VACINADO && resposta.dados.status!= EMCURSO ){
        int index_enfermeiro=-1;
        char CS_cidadao[102]="CS"; //guardamos as iniciais CS na variável cs
        strcat(CS_cidadao,resposta.dados.cidadao.localidade); //queremos juntar CS ao início da localidade

        for(int i=0; i!=db->num_enfermeiros && index_enfermeiro==-1; i++)
            if(strcmp(db->enfermeiros[i].CS_enfermeiro,CS_cidadao) == 0)
                index_enfermeiro=i;

    //       • Se não houver centro de saúde, ou não houver nenhum enfermeiro no centro de saúde correspondente => status = NAOHAENFERMEIRO;
        if(index_enfermeiro==-1){
    ...
    //       • Se há enfermeiro, mas este não tiver disponibilidade => status = AGUARDAR.
        else if(db->enfermeiros[index_enfermeiro].disponibilidade==0){
            resposta.dados.status = AGUARDAR;
        }
        
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
        if(index_enfermeiro!=-1){
            sucesso("S5.2) Enfermeiro do CS %s encontrado, disponibilidade=%d, status=%d", resposta.dados.cidadao.localidade, db->enfermeiros[index_enfermeiro].disponibilidade, resposta.dados.status);
    ...
    // S5.3) Caso o enfermeiro esteja disponível, procura uma vaga para vacinação na BD Vagas. Para tal, chama a função reserva_vaga(Index_Cidadao, Index_Enfermeiro) usando os índices do Cidadão e do Enfermeiro nas respetivas BDs:
            if(db->enfermeiros[index_enfermeiro].disponibilidade!=0){
                sem_mutex_up();
                vaga_ativa = reserva_vaga(index_cidadao, index_enfermeiro);
                sem_mutex_down();
    ...
    // S5.4) caso contrário, chama a função envia_resposta_cidadao(), que envia a resposta ao Cidadão;
        ...
        db->cidadaos[index_cidadao].PID_cidadao = -1;
...
void vacina() {
    ...
    int pid = fork();
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    exit_on_error(pid, "S6.1) Não foi possível criar um novo processo");
...
void servidor_dedicado() {
    ...
    sem_mutex_down();
    int index_enfermeiro=db->vagas[vaga_ativa].index_enfermeiro;
    int index_cidadao=db->vagas[vaga_ativa].index_cidadao;
    db->enfermeiros[index_enfermeiro].disponibilidade=0;
    sem_mutex_up();
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    ...
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    sucesso("S7.4) Vacina em curso para o cidadão %d, %s, e com o enfermeiro %d, %s na vaga %d", db->cidadaos[index_cidadao].num_utente, db->cidadaos[index_cidadao].nome, db->enfermeiros[index_enfermeiro].ced_profissional, db->enfermeiros[index_enfermeiro].nome, vaga_ativa);
    ...
    db->cidadaos[index_cidadao].estado_vacinacao++;
    ...
    db->enfermeiros[index_enfermeiro].nr_vacinas_dadas++;
    // S7.6) Atualiza os dados do enfermeiro (coloca disponibilidade=1) na BD de Enfermeiros;
    db->enfermeiros[index_enfermeiro].disponibilidade = 1;
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    sucesso("S7.6) Cidadão atualizado na BD para estado_vacinacao=%d, Enfermeiro atualizado na BD para nr_vacinas_dadas=%d e disponibilidade=%d", db->cidadaos[index_cidadao].estado_vacinacao, db->enfermeiros[index_enfermeiro].nr_vacinas_dadas, db->enfermeiros[index_enfermeiro].disponibilidade);
    sem_mutex_up();
...
int reserva_vaga(int index_cidadao, int index_enfermeiro) {
    ...
    for(int i=0; i!=MAX_VAGAS && vaga_ativa==-1; i++)
...
void cancela_pedido() {
    ...
    int index_cidadao = -1;
    int index_vaga = -1;
    for(int i=0; i!= MAX_VAGAS && index_cidadao==-1; i++){
        int aux = db->vagas[i].index_cidadao;
        if(db->cidadaos[aux].num_utente==mensagem.dados.num_utente){
            index_cidadao = aux;
            index_vaga = i;
        }
    }
    ...
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    sucesso("S10.1) Foi encontrada a sessão do cidadão %d, %s na sala com o index %d", mensagem.dados.num_utente, mensagem.dados.nome, index_vaga);

    // S10.2) Envia um sinal SIGTERM ao processo Servidor Dedicado (filho) que está a tratar da vacinação;
    kill(db->vagas[index_vaga].PID_filho,SIGTERM);
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    sucesso("S10.2) Enviado sinal SIGTERM ao Servidor Dedicado com PID=%d", db->vagas[index_vaga].PID_filho);
    sem_mutex_up();
