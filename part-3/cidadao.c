...
void cria_mensagem() {
    ...
    scanf("%d",&mensagem.dados.num_utente); //guardar o número de utente na variável num_utente
    
    printf("Insira o seu nome:\n"); 
    my_fgets(mensagem.dados.nome,100,stdin); //guardar o nome na variável nome
...
void trata_resposta_servidor(){
    ...
    switch(resposta.dados.status){
        ...
        case EMCURSO:
        // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
            sucesso("C5.2) A vacinação do utente %d, %s já está em curso", resposta.dados.cidadao.num_utente, resposta.dados.cidadao.nome);
            exit(0);
            break;
...
void cancela_pedido(int sinal) {
    ...
    mensagem.dados.pedido=CANCELADA;
    envia_mensagem_servidor();
